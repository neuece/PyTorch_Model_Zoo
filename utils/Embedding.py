...
class ConstEmbedding(nn.Module):
    def __init__(self, pretrained_embedding, padding_idx=0):
     ...
     # The embedding matrix should be always on cpu
...
# trainable embeddings, saved on GPU if possible
...
def load_predtrained_emb_zero(path, words_dic, padding=False):
    ...
    with open(path, encoding='utf-8') as f:
        for line in f.readlines():
            ...
            if len(line) <= 1:
                print("error: load_predtrained_embedding  -> len(line) <= 1")
                break
            else:
                embeding_dim = len(line) - 1
                break
    vocab_size = len(words_dic)
    print("The word size is ", vocab_size)
    print("The dim of predtrained embedding is ", embeding_dim)

    embedding = np.zeros((vocab_size, embeding_dim))
    in_word_list = []
    in_words = 0
